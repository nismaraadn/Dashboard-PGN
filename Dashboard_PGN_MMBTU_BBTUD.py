...
# 1. Judul Aplikasi
# -----------------------------------------------------
st.markdown("<h1 class='big-title'>Dashboard PGN</h1>", unsafe_allow_html=True)
st.write(" ")
...
dashboard_choice = st.sidebar.radio(
    "Jenis Dashboard",
    ["Dashboard Usage", "Dashboard Monitoring"]
)
...
# 7. Dashboard Usage
# -----------------------------------------------------
if dashboard_choice == "Dashboard Usage":
    ...
    if (
        ...
        with tabs_usage[1]:
            ...
            def create_usage_panel(panel_key, df_original):
                ...
                # Convert units if BBTUD is selected
                if selected_unit == "BBTUD":
                    filtered_df['Penggunaan'] = filtered_df['Penggunaan'] / 1000
                    unit_label = "BBTUD"
                else:
                    unit_label = "MMBtu"
                ...
                stats_data = [{
                    'Total': f"{total_val:,.2f} {unit_label}",
                    'Average': f"{avg_val:,.2f} {unit_label}",
                    'Minimum': f"{min_val:,.2f} {unit_label}",
                    'Maximum': f"{max_val:,.2f} {unit_label}",
                }]
                ...
                df_display['Penggunaan'] = df_display['Penggunaan'].apply(lambda x: f"{float(x):,.2f}")
        ...
        with tabs_usage[2]:
            ...
            def create_weekend_weekday_panel(panel_key, df_original):
                ...
                selected_unit = st.radio(
                    ...
                    options=["MMBTU", "BBTUD"],
                    horizontal=True,
                    key=f"weekend_unit_selection_{panel_key}"
                )
                ...
                for cat in categories:
                    ...
                    if cat_df.empty:
                        stats_data.append({
                            'Kategori': cat,
                            'Total': f'0 {unit_label}',
                            'Average': f'0 {unit_label}',
                            'Minimum': f'0 {unit_label}',
                            'Maximum': f'0 {unit_label}'
                        })
                    else:
                        ...
                        stats_data.append({
                            'Kategori': cat,
                            'Total': f"{total_cat:,.2f} {unit_label}",
                            'Average': f"{avg_cat:,.2f} {unit_label}",
                            'Minimum': f"{min_cat:,.2f} {unit_label}",
                            'Maximum': f"{max_cat:,.2f} {unit_label}"
                        })
        ...
        with tabs_usage[3]:
            ...
            def convert_to_bbtud(mmbtu_value, year):
                """
                Convert annual MMBTU value to average BBTUD
                Using average days in a year (365 or 366 for leap years)
                """
                def is_leap_year(year):
                    return year % 4 == 0 and (year % 100 != 0 or year % 400 == 0)
                
                days_in_year = 366 if is_leap_year(year) else 365
                return (mmbtu_value / 1000) / days_in_year
            ...
            def annual_panel(panel, panel_side):
                ...
                with panel:
                    ...
                    # Add unit selection radio button
                    selected_unit = st.radio(
                        f"Pilih Satuan ({panel_side.capitalize()})",
                        ["MMBTU", "BBTUD"],
                        horizontal=True,
                        key=f"annual_{panel_side}_unit"
                    )
                    ...
                    if filtered_df.empty:
                        ...
                        # Convert to BBTUD if selected
                        if selected_unit == "BBTUD":
                            yearly_stats['Total'] = yearly_stats.apply(
                                lambda row: convert_to_bbtud(row['Total'], row['Tahun']),
                                axis=1
                            )
                            yearly_stats['Average'] = yearly_stats.apply(
                                lambda row: convert_to_bbtud(row['Average'], row['Tahun']),
                                axis=1
                            )
                            yearly_stats['Minimum'] = yearly_stats.apply(
                                lambda row: convert_to_bbtud(row['Minimum'], row['Tahun']),
                                axis=1
                            )
                            yearly_stats['Maximum'] = yearly_stats.apply(
                                lambda row: convert_to_bbtud(row['Maximum'], row['Tahun']),
                                axis=1
                            )
                        ...
                        fig.update_layout(
                            xaxis_title="Year",
                            yaxis_title=f"Total Consumption ({unit_label})",
                            showlegend=False
                        )
                        st.plotly_chart(fig, use_container_width=True)
                        ...
                        insight_text = (f"Tahun dengan konsumsi gas tertinggi adalah *{int(max_year_val)}* "
                                    f"dengan total *{max_total:,.2f} {unit_label}*. "
                                    f"Tahun dengan konsumsi gas terendah adalah *{int(min_year_val)}* "
                                    f"dengan total *{min_total:,.2f} {unit_label}*.")
                        st.write("#### Insight")
                        ...
                        st.write(f"#### Yearly Gas Consumption Statistics ({unit_label})")
                        ...
                        # Convert to BBTUD if selected
                        if selected_unit == "BBTUD":
                            contributor_data['Total_Penggunaan'] = contributor_data.apply(
                                lambda row: convert_to_bbtud(row['Total_Penggunaan'], end_year),
                                axis=1
                            )
                        ...
                        st.dataframe(
                            ...
                            column_config={
                                ...
                                "Total_Penggunaan": f"Total Usage ({unit_label})"
                            },
                            hide_index=True
                        )
        ...
        with tabs_usage[4]:
            ...
            if enable_comparison:
                ...
                def create_segment_panel(panel_key):
                    ...
                    # Tambahkan radio button untuk pemilihan satuan
                    selected_unit = st.radio(
                        "Pilih Satuan",
                        ["MMBTU", "BBTUD"],
                        horizontal=True,
                    ...
                    # Konversi nilai berdasarkan satuan yang dipilih
                    df_seg_filtered_copy = df_seg_filtered.copy()
                    if selected_unit == "BBTUD":
                        days = get_days_in_month(selected_year, selected_month)
                        df_seg_filtered_copy['Nilai'] = df_seg_filtered_copy['Nilai'] / 1000 / days
                    
                    # Calculate segment statistics
                    segment_stats = df_seg_filtered_copy.groupby('Segment').agg(
                    ...
                    fig = px.pie(
                        ...
                        names='Segment',
                        values='Total_Nilai',
                        title=f"Distribution of Values by Segment - {selected_cm}, {selected_month}, {selected_year} ({unit_label})",
                        hole=0.4,
                        template='plotly_white',
                        height=600,
                    ...
                    fig.update_traces(
                        ...
                        hovertemplate=(
                            ...
                            f'<b>Total Nilai ({unit_label}):</b> %{{value:,.2f}}<br>'
                    ...
                    top_segment_data = [
                        {
                            ...
                            'Total Konsumsi': f"{numeric_konsumsi.loc[max_idx, 'Total_Konsumsi']:,.2f} {unit_label}"
                        },
                        {
                            'Indikator': 'Segmen Terendah',
                            'Segment': numeric_konsumsi.loc[min_idx, 'Segment'],
                            'Total Konsumsi': f"{numeric_konsumsi.loc[min_idx, 'Total_Konsumsi']:,.2f} {unit_label}"
                        }
                    ]
                    ...
                    df_segment_filtered = df_seg_filtered_copy.copy()
                    if selected_segment != 'All':
                    ...
                    if not df_segment_filtered.empty:
                        ...
                        customer_stats['Total_Konsumsi'] = customer_stats['Total_Konsumsi'].apply(lambda x: f"{x:,.2f} {unit_label}")
        ...
        with tabs_usage[5]:
            ...
            def get_days_in_month(year, month):
                ...
                days_in_month = {
                    1: 31,  # January/Januari
                    2: 29 if is_leap_year(year) else 28,  # February/Februari
                    3: 31,  # March/Maret
                    4: 30,  # April
                    5: 31,  # May/Mei
                    6: 30,  # June/Juni
                    ...
                    8: 31,  # August/Agustus
                    9: 30,  # September
                    10: 31,  # October/Oktober
                    11: 30,  # November
                    12: 31   # December/Desember
                }
                
                # Handle direct month name input
                if isinstance(month, str):
                    month_num = month_to_num.get(month)
                    if month_num is None:
                        # Add debugging information
                        st.error(f"Invalid month name: {month}. Valid months are: {', '.join(sorted(month_to_num.keys()))}")
                        return 30  # Default fallback
                else:
                    month_num = month
                    
                if month_num not in days_in_month:
                    st.error(f"Invalid month number: {month_num}")
                    return 30  # Default fallback
            ...
            def convert_to_bbtud(value, year, month=None):
                """
                Convert MMBTU to BBTUD
                If month is None or 'All', calculate based on full year
                Otherwise calculate based on specific month
                """
                try:
                    ...
                    # Convert MMBTU to BBTUD (dividing by 1000 and by number of days)
                    bbtud = value / 1000 / days
                    return bbtud
                    
                except Exception as e:
                    st.error(f"Error in BBTUD conversion: {str(e)}")
            ...
            if enable_comparison:
                ...
                def create_sektor_panel(panel_key):
                    ...
                    # Add unit selection with unique key
                    selected_unit = st.radio(
                        "Pilih Satuan",
                        ["MMBTU", "BBTUD"],
                        horizontal=True, 
                        key=f"unit_selection_{panel_key}_comparison"
                    )
                    ...
                    # Convert values if BBTUD is selected
                    # Convert values if BBTUD is selected
                    if selected_unit == "BBTUD":
                        filtered_df['Nilai'] = filtered_df.apply(
                            lambda row: convert_to_bbtud(
                                row['Nilai'], 
                                row['Tahun'], 
                                row['Bulan'] if selected_month != 'All' else None
                            ),
                            axis=1
                        )
                    # Pie Chart Sektor
                    ...
                    unit_suffix = "BBTUD" if selected_unit == "BBTUD" else "MMBtu"
                    
                    fig = px.pie(
                        sector_stats,
                        names='Sektor',
                        values='Total_Nilai',
                        title=f"Distribution of Values by Sektor - {selected_cm}, {selected_month}, {selected_year} ({unit_suffix})",
                        hole=0.4,
                        template='plotly_white',
                        height=600,
                        custom_data=['Jumlah_Pelanggan']
                    ...
                    fig.update_traces(
                        ...
                        hovertemplate=(
                            '<b>Sektor:</b> %{label}<br>'
                            f'<b>Total Nilai ({unit_suffix}):</b> %{{value:,.2f}}<br>'
                            '<b>Jumlah Pelanggan:</b> %{customdata[0]}<extra></extra>'
                        )
                    )
                    ...
                    sector_stats_display.rename(columns={
                        'Total_Nilai': f'Total_Konsumsi_{unit_suffix}',
                    }, inplace=True)
                    ...
                    # Format
                    sector_stats_display[f'Total_Konsumsi_{unit_suffix}'] = sector_stats_display[f'Total_Konsumsi_{unit_suffix}'].apply(lambda x: f"{x:,.2f}")
                    ...
                    if not df_sector_filtered.empty:
                        ...
                        df_customers['Total_Konsumsi'] = df_customers['Total_Konsumsi'].apply(lambda x: f"{x:,.2f} {unit_suffix}")
                ...
                if df_sector.empty:
                    ...
                    selected_unit = st.radio(
                        ...
                        ["MMBTU", "BBTUD"],
                        horizontal=True,
                        key="unit_selection_single_panel"  # Changed key to be unique
                    )
                    
                    # Pilih Tahun
                    tahun_options = sorted(df_sector['Tahun'].dropna().unique())
                    selected_year = st.selectbox("Pilih Tahun", tahun_options, index=0, key="sector_year_usage_single")  # Added _single
                    ...
                    selected_cm = st.selectbox(
                        ...
                        cm_options_list, 
                        index=0 if len(cm_options_list) > 0 else -1, 
                        key="sector_cm_usage_single"  # Added _single
                    )
                    ...
                    selected_month = st.selectbox(
                        ...
                        sector_month_options, 
                        index=0, 
                        key="sector_month_usage_single"  # Added _single
                    )
                    ...
                    if filtered_df.empty:
                        ...
                        # Add unique key for plotly chart
                        st.plotly_chart(fig, use_container_width=True, key="pie_chart_sector_single_panel")  # Changed key
                        ...
                        selected_sektor = st.selectbox(
                            ...
                            sektor_list, 
                            index=0, 
                            key="sector_filter_usage_single"  # Added _single
                        )
                        ...
                        sector_ranking_filter = st.selectbox(
                            ...
                            ranking_options, 
                            index=0, 
                            key="sector_customer_ranking_usage_single"  # Added _single
                        )
                        ...
                        if not df_sector_filtered.empty:
                            ...
                            st.info("Tidak ada data pelanggan untuk sektor/filter terpilih.")                  
        # -------------------------------------------------
        # 8. TAB: SEKTOR (Sudah termasuk di tabs_usage[7])
        # -------------------------------------------------
        # Sudah termasuk dalam tabs_usage[7], tidak perlu tambahan lagi
        ...
        with tabs_usage[6]:
            ...
            # Function to convert MMBTU to BBTUD
            def convert_to_bbtud(value, year, month):
                days = get_days_in_month(year, month)
            ...
            def create_comparison_section(key_prefix):
                ...
                selected_unit = st.radio(
                    ...
                    key=f"unit_selection_{key_prefix}"
                )
                # Pastikan kolom Tahun numeric
                df_sector_compare['Tahun'] = pd.to_numeric(df_sector_compare['Tahun'], errors='coerce')
                ...
                for yr in range(year_range[0], year_range[1] + 1):
                    ...
                    if not df_year.empty:
                        ...
                        # Convert to BBTUD if selected
                        if selected_unit == "BBTUD":
                            monthly_grouped['Nilai'] = monthly_grouped.apply(
                                lambda row: convert_to_bbtud(row['Nilai'], yr, row['Bulan']),
                                axis=1
                            )
                ...
                # Convert totals to BBTUD if selected
                if selected_unit == "BBTUD":
                    # For yearly totals, we'll use average of monthly BBTUD values
                    total_awal_monthly = df_awal.groupby('Bulan')['Nilai'].sum()
                    total_akhir_monthly = df_akhir.groupby('Bulan')['Nilai'].sum()
                    
                    total_awal_bbtud = sum(
                        convert_to_bbtud(val, year_range[0], month) 
                        for month, val in total_awal_monthly.items()
                    ) / len(total_awal_monthly) if not total_awal_monthly.empty else 0
                    
                    total_akhir_bbtud = sum(
                        convert_to_bbtud(val, year_range[1], month) 
                        for month, val in total_akhir_monthly.items()
                    ) / len(total_akhir_monthly) if not total_akhir_monthly.empty else 0
                    
                    total_awal = total_awal_bbtud
                    total_akhir = total_akhir_bbtud
                ...
                # Convert averages to BBTUD if selected
                if selected_unit == "BBTUD":
                    avg_awal_monthly = df_awal.groupby('Bulan')['Nilai'].mean()
                    avg_akhir_monthly = df_akhir.groupby('Bulan')['Nilai'].mean()
                    
                    avg_awal = sum(
                        convert_to_bbtud(val, year_range[0], month) 
                        for month, val in avg_awal_monthly.items()
                    ) / len(avg_awal_monthly) if not avg_awal_monthly.empty else 0
                    
                    avg_akhir = sum(
                        convert_to_bbtud(val, year_range[1], month) 
                        for month, val in avg_akhir_monthly.items()
                    ) / len(avg_akhir_monthly) if not avg_akhir_monthly.empty else 0
                ...
                for m in month_order:
                    ...
                    # Convert to BBTUD if selected
                    if selected_unit == "BBTUD":
                        awal_val = convert_to_bbtud(awal_val, year_range[0], m)
                        akhir_val = convert_to_bbtud(akhir_val, year_range[1], m)
                ...
                if df_contributor.empty:
                    ...
                    # Function to calculate yearly total with unit conversion
                    def calculate_yearly_total(df_, year):
                        if selected_unit == "BBTUD":
                            # Calculate BBTUD for each month and take the average
                            yearly_total = {}
                            for name in df_.index.get_level_values(0).unique():
                                monthly_values = []
                                for month in month_order:
                                    try:
                                        value = df_.loc[(name, month)]
                                        bbtud_value = convert_to_bbtud(value, year, month)
                                        monthly_values.append(bbtud_value)
                                    except KeyError:
                                        continue
                                if monthly_values:
                                    yearly_total[name] = sum(monthly_values) / len(monthly_values)
                                else:
                                    yearly_total[name] = 0
                            return yearly_total
                        else:
                            return df_.groupby('Nama Pelanggan').sum()

                    yearly_totals_awal = calculate_yearly_total(awal_df_, year_range[0])
                    yearly_totals_akhir = calculate_yearly_total(akhir_df_, year_range[1])
                    ...
                    # Convert Total Konsumsi to BBTUD if selected
                    if selected_unit == "BBTUD":
                        total_months = len(month_order)
                        contributor_grouped['Total Konsumsi'] = contributor_grouped['Total Konsumsi'] / 1000 / (365 / 12)  # Average days per month
                    ...
                    # Format angka
                    contributor_grouped['Total Konsumsi'] = contributor_grouped['Total Konsumsi'].apply(lambda x: f"{x:,.2f} {selected_unit}")
                    contributor_grouped['Tahun Awal'] = contributor_grouped['Tahun Awal'].apply(lambda x: f"{x:,.2f} {selected_unit}")
                    contributor_grouped['Tahun Akhir'] = contributor_grouped['Tahun Akhir'].apply(lambda x: f"{x:,.2f} {selected_unit}")
                    contributor_grouped['Perubahan Absolut'] = contributor_grouped['Perubahan Absolut'].apply(lambda x: f"{x:,.2f} {selected_unit}")
...
# -----------------------------------------------------
elif dashboard_choice == "Dashboard Monitoring":
    ...
    if (
        ...
        with tabs_monitoring[0]:
            ...
            def create_monitoring_panel(df_bulanan_all, panel_key):
                ...
                def convert_to_selected_unit(df, unit):
                    if unit == 'BBTUD':
                        ...
                        # Function to handle potential leap years
                        def get_days(month, year):
                            if month == 'Feb' and year % 4 == 0 and (year % 100 != 0 or year % 400 == 0):
                                return 29
                            return month_days.get(month, 30)
                        
                        # Convert columns to BBTUD
                        columns_to_convert = [
                            'Real_Monthly', 'Kmin', 'Kmax', 'Nominasi', 'RKAP_Actual',
                            'RKAP_Ac', 'Real_Ac_Monthly'
                        ]
                        
                        for col in columns_to_convert:
                            if col in df.columns:
                                df[col] = df.apply(
                                    lambda row: row[col] / 1000 / get_days(row['Bulan'].split('-')[0], row['Tahun']) 
                                    if pd.notnull(row[col]) else row[col], 
                                    axis=1
                                )
                    return df
                ...
                filtered_bulanan = convert_to_selected_unit(filtered_bulanan, selected_unit)
                ...
                # Inside the create_monitoring_panel function, before creating the plot:
                # Add display options
                ...
                if not filtered_bulanan.empty:
                    ...
                    title_text = (f'Grafik Bulanan: {selected_pelanggan} - {year_range_text} ({selected_unit})'
                                if selected_pelanggan != "All Pelanggan"
                                else f'Grafik Bulanan: All Pelanggan - {year_range_text} ({selected_unit})')
                    ...
                    # Rest of the code (display tables, download functionality, etc.) remains the same
                    columns_to_display = [
                    ...
                    st.download_button(
                        label=f"Download Data Bulanan (Excel) - {selected_unit}",
                        data=excel_buffer,
                        file_name=f"Monthly_Data_{selected_pelanggan}_{year_range_text}_{selected_unit}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        ...
        with tabs_monitoring[1]:
            ...
            def create_monitoring_panel(df_harian_all, panel_key=""):
                ...
                def convert_values(df, unit):
                    df = df.copy()
                    if unit == 'BBTUD':
                        columns_to_convert = ['Real_Daily', 'Kmin', 'Kmax', 'Nominasi', 'RKAP_Actual']
                        for col in columns_to_convert:
                            if col in df.columns:
                                df[col] = df[col] / 1000
                    return df
...
# Inside the create_monitoring_panel function, before creating the plot:
                ...
                if not filtered_harian.empty:
                    ...
                    def create_trace(y_column, name, color, show_on_second_y=False):
                        hover_text = filtered_harian.apply(
                            lambda row: f"Nama Pelanggan: {row['Nama Pelanggan']}<br>" + 
                                      ...
                                      (" %" if name in ['RR%', 'Ac%'] else f" {selected_unit}"),
                            axis=1
                        )
                    ...
                    title_str = f"Grafik Harian: {selected_pelanggan_daily} - {start_date} s/d {end_date} ({selected_unit})"
                ...
                # --------------------------------------------
                st.write(f"#### Data Harian ({selected_unit})")
                ...
                st.download_button(
                    label=f"Download Full Data as Excel ({selected_unit})",
                    data=excel_data,
                    file_name=f"data_harian_full_{selected_unit}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
        ...
        # --------------------------------------------
                ...
                # --------------------------------------------
                st.write(f"#### Ringkasan Data Harian ({selected_unit})")
                if not filtered_harian.empty:
                    numeric_cols = [
                ...
                # TABEL KONTRIBUTOR
                # --------------------------------------------
                st.write(f"### Tabel Kontributor Harian ({selected_unit})")
